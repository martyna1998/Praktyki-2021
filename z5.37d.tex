...
\kategoria{Wikieł/Z5.37d}

\zadStart{Zadanie z Wikieł Z 5.37 d) moja wersja nr [nrWersji]}
...
$$f'(x) = 6(x + [c])^5 + [d],\quad f''(x) = 30(x + [c])^4$$
i określamy ich dziedziny: $\quad \mathcal{D}_{f'}=\mathcal{D}_{f''}=\mathbb{R}$. \\
\item Znajdujemy miejsca zerowe $f''$: 
$$f''(x)=0 \Leftrightarrow x=-[c] (k=4)$$
\item Badamy znak $f''$ po obu stronach miejsc zerowych. \\
	\begin{enumerate}
	\item $f''(x) > 0 \Leftrightarrow x \in \mathbb{R}\backslash \{-[c]\}$\\
	\item $f''(x) < 0 \Leftrightarrow x \in \emptyset$
	\end{enumerate}
\end{enumerate}
Tym samym w sąsiedztwie punktu $x=-[c]$ druga pochodna nie zmienia znaku, a więc wykres funkcji nie ma punktów przegięcia.
\rozwStop
...
D. Współrzędne punktów przegięcia to: $(-[c],[e])$.
\testStop
