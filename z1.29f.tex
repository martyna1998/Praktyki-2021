...
\kategoria{Wikieł/Z1.29f}
\zadStart{Zadanie z Wikieł Z 1.29 f) moja wersja nr [nrWersji]}
%[p1]:[2,4,6,8,10,12]
%[a]=random.randint(1,25)
%[b]=random.randint(2,25)
%[c]=random.randint(2,25)
Zbadać parzystość (nieparzystość) funkcji: $ f(x)=[b]sgn(x)\cdot([c]x^{[p1]}+[a])$.
\zadStop
\rozwStart{Wojciech Przybylski}{Laura Mieczkowska}
$$f(-x)=[b]sgn(-x)\cdot([c](-x)^{[p1]}+[a])=-[b]sgn(x)\cdot([c]x^{[p1]}+[a])=-f(x)$$
\rozwStop
\odpStart
